...
namespace BizzyBeeGames.DotConnect
{
	public class LevelCompletePopup : Popup
	{
		...
		public override void OnShowing(object[] inData)
		{
			...
			//{
   //             numMovesText.text = string.Format("Вы прошли уровень за <color=#{0}>{1}</color> " + GetMoveWord(numMoves) + "!", ColorUtility.ToHtmlStringRGB(numMovesColor), numMoves);
   //         }
			//else
			//{
        ...
        string GetMoveWord(int numMoves)
        {
            int lastDigit = numMoves % 10;
            int lastTwoDigits = numMoves % 100;
